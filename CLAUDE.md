...
対になる相手が exe でないものは、この限りではない。
`.claude/skills/xyzzy-lisp/references/` は `reference/reference.xml` と対で、
原稿の方が git に入っているので版がずれない。clone しただけで引けることを取って
追跡する。古くなっていないかは `misc/dev.sh reference` が見る。
