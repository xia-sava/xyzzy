...
protected:
  ...
  BOOL range_command (UINT, int, UINT, LONG, const WCHAR *) const;
  ...
  void set_margin_text (UINT, LONG, const WCHAR *) const;
  void set_margin (UINT, UINT, LONG, int, int, int, const WCHAR *) const;
  LONG parse_margin_text (UINT, const WCHAR *) const;
  ...
  int check_margin_text (UINT, LONG &, LONG, LONG, const WCHAR *) const;
  BOOL notify_spin (NMHDR *, const WCHAR *);
  BOOL print_setup ();
  int preview ();
  BOOL set_font ();
  void set_font_face () const;
  int get_result (int);
  ...
  int find_history (UINT, const WCHAR *);
  static int find_menu_text (HMENU, int, WCHAR *, int);
