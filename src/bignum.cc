...
{
  return static_cast <long> (br_len ? (br_len - 1) * BR_SHIFT + ::log2_floor (br_data[br_len - 1]) : 0);
}
...
bignum_rep::howlong () const
{
  ...
  return static_cast <long> ((br_len - 1) * BR_SHIFT + ::log2_floor (x));
}
...
{
  return (br_len + 1) * BR_SHIFT / (::log2_floor (base) - 1) + 16;
}
...
ato_bignum_rep (bignum_rep *&br, const Char *p, int pl, int radix)
{
  ...
  const Char *pe = p + pl;
  int width = static_cast <int> (pl * log2_floor (radix) / BR_SHIFT + 1);
  bignum_rep *rep;
