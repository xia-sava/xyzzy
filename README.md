...
xyzzy 0.2.2 系列は以下の観点を重視したバージョンです。
  ...
  * Lisp 動作における xyzzy 0.2.2.235 との後方互換性
    * xyzzy は過去に作られた資産のうちメンテされていないものが結構ありますが、
      これらを修正せずに動作できることを原則とします
    * ただし現代化のために必要な場合は、次の両方を満たすことを条件に
      この原則を緩めます
      * リポジトリに含まれる Lisp コードとの整合が取れていること
      * 利用者の Lisp 資産に影響する変更は、その内容と移行方法を
        [docs/incompatible-changes.md](docs/incompatible-changes.md)
        に記載すること
...
他の派生版との違いは以下のとおりです。
  ...
  * unicode 版と同じく、文字を Unicode の符号位置で持ちます。
    そのため文字を数値で扱う Lisp は 0.2.2.235 と互換ではありません。
    何が変わったかは [docs/incompatible-changes.md](docs/incompatible-changes.md)
    に書いてあります。
