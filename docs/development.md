...
期待値は **ビルド 0 エラー 0 警告**、バイトコンパイルは `lisp/` 直下の `.l` と同じ数の
`.lc`、**`test` は OLE の 4 件だけが落ちてあとは全部通る**、**`test visible` はその 4 件が
通る**。警告が出たら回帰なので放置しないこと。個数そのものは `.l` と試験が増えるたびに
変わるので、**数を覚えず「落ちてよいのは OLE の 4 件だけ」で見る**。

バイトコンパイルの数は `dev.sh` が `lisp/*.lc` を**直下だけ**数えたもの。`lisp/wip/` を
含めて再帰で数えると増える。数え方が違うだけで、作られるものは同じ。
...
CI では**全件が通るのが期待値**。手元と違って OLE の 4 件も通る。ランナーでは
...
`src/comctl6.manifest` を `projects/xyzzy.vcxproj` からだけ参照している。
`<Manifest Include>` は複数書けて mt.exe が一本に結合するので、DPI 認識・
`longPathAware` と同じ埋め込みマニフェストに同居する。`xyzzycli` と `xyzzyenv` は
窓をほとんど作らないので参照しない。

### 効いている版は、載っている版では判らない

5.82 も 6.x も `WinSxS` の下にあり、comdlg32 などが自前のアクティブ化コンテキストで
6.x を引き込むので、**マニフェストが無くても両方が載る**。効いている版は
**窓のクラスを登録したモジュール**に出る。`misc/verify/comctlprobe.py` がそれを読む。

### 版 6 に依っている箇所

- **一覧の並べ替えの矢印**（`src/privctrl/listviewex.cc`）。`HDF_SORTUP` /
  `HDF_SORTDOWN` は版 6 でしか効かない。マニフェストを外すと、ファイラと
  バッファ選択の見出しから矢印が**無言で消える**
- **標準コントロールの差し替え**（`src/stdctl.cc`）。版 6 のアセンブリは
  Button / Static / Listbox / ScrollBar / Edit / Combobox のクラス名を
  `CreateWindowEx` の時点で版付きの別名へ引き直すので、**同名で登録し直す手は
  届かない**。窓ごとに `WH_CBT` で捕まえて差し替えている
- **タブのスピンの向き**は実装が決める。縦置きのタブバーには縦向きのスピンが付く

### ダイアログの寸法は変わらない

`src/xyzzy.rc` のテンプレートも Lisp の `create-dialog` も、基準単位がフォント由来
なので**版で変わらない**。実測でも、プロパティシート（共通設定・ローカル設定）が
余白の分だけ数画素大きくなるだけで、他のダイアログは 1 画素も動かなかった。
コントロールの中身は版 6 の方が少し詰まるので、切れていた文字がむしろ収まる。

見た目を確かめるときは 96 dpi と拡大された画面の両方で見る。画面の設定を変えずに
96 dpi で走らせる手は `misc/verify/README.md` にある。
...
**どのファイルで止まったかは `lisp/*.lc` の更新時刻で分かる。** `makelc` は名前順に
回るので、更新された最後のものの次がそれ。
...
### 終端マクロ文字を Ctrl と組むときは逃がす

`#\C-,` は読めない。`,` `;` `"` `(` `)` は終端マクロ文字で、`extended_token`
（`src/lread.cc`）がそこでトークンを切る。修飾を剥がすと中身が空になり
`Einvalid_char_code_syntax` になるので、バイトコンパイル中なら msgbox で止まる。

```lisp
(global-set-key #\C-\, 'contract-region)
```

**キーそのものは打てる。** `src/gen-ctab.cc` の `pseudo_char2ctl_table` に載っていれば、
キー入力（`kbd.cc`）と文字リテラルの読み取り（`lread.cc`）が同じ表を見るので、
綴り方だけの問題である。`.` `/` `-` `0`-`9` は構成文字なので逃がさなくてよい。
...
**窓自身の DC から撮る (`GetDC` + `BitBlt`)**: 前面に出す必要がなく、遮蔽とも無関係で、
`WM_PRINTCLIENT` に応じない窓でも撮れる。**本体ウィンドウはこの手なら撮れる。**
子ウィンドウは自分の DC を持つので、奥から順に貼り重ねる。枠とタイトルバーは
非クライアント領域なので写らない。
